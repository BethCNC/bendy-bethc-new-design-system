...
const responsiveMobile = JSON.parse(fs.readFileSync(responsiveMobilePath, 'utf8'));
const responsiveTablet = JSON.parse(fs.readFileSync(responsiveTabletPath, 'utf8'));
const responsiveDesktop = JSON.parse(fs.readFileSync(responsiveDesktopPath, 'utf8'));
const primitivesLight = JSON.parse(fs.readFileSync(primitivesLightPath, 'utf8'));
const primitivesDark = JSON.parse(fs.readFileSync(primitivesDarkPath, 'utf8'));
...
function generateSemanticCSS(mapped, primitivesLight, primitivesDark) {
  let css = `/**
 * SEMANTIC TOKENS
...
function generateUtilitiesCSS(mapped, mobile, tablet, desktop) {
  let css = `/**
 * UTILITY CLASSES
...
function generatePrimitivesCSS(primitivesLight, mobile, tablet, desktop) {
  let css = `/**
 * PRIMITIVE TOKENS
...
`;
  ...
  // Mobile typography
  if (mobile.fontsize && mobile.lineheight) {
    css += '  /* Mobile Typography */\n';
    
    // Display tokens
    if (mobile.fontsize.display?.display) {
      css += `  --font-display-display-mobile: ${mobile.fontsize.display.display}px;\n`;
    }
    if (mobile.lineheight.display?.display) {
      css += `  --line-display-display-mobile: ${mobile.lineheight.display.display}px;\n`;
    }
    
    // Heading tokens
    if (mobile.fontsize.heading) {
      Object.entries(mobile.fontsize.heading).forEach(([key, value]) => {
        css += `  --font-heading-${key.toLowerCase()}-mobile: ${value}px;\n`;
      });
    }
    if (mobile.lineheight.heading) {
      Object.entries(mobile.lineheight.heading).forEach(([key, value]) => {
        css += `  --line-heading-${key.toLowerCase()}-mobile: ${value}px;\n`;
      });
    }
    
    // Title tokens
    if (mobile.fontsize.title) {
      Object.entries(mobile.fontsize.title).forEach(([key, value]) => {
        css += `  --font-title-${key}-mobile: ${value}px;\n`;
      });
    }
    if (mobile.lineheight.title) {
      Object.entries(mobile.lineheight.title).forEach(([key, value]) => {
        css += `  --line-title-${key}-mobile: ${value}px;\n`;
      });
    }
    
    // Body tokens
    if (mobile.fontsize.body) {
      Object.entries(mobile.fontsize.body).forEach(([key, value]) => {
        css += `  --font-body-${key}-mobile: ${value}px;\n`;
      });
    }
    if (mobile.lineheight.body) {
      Object.entries(mobile.lineheight.body).forEach(([key, value]) => {
        css += `  --line-body-${key}-mobile: ${value}px;\n`;
      });
    }
  }
  
  // Tablet typography
  if (tablet.fontsize && tablet.lineheight) {
    css += '\n  /* Tablet Typography */\n';
    
    // Display tokens
    if (tablet.fontsize.display?.display) {
      css += `  --font-display-display-tablet: ${tablet.fontsize.display.display}px;\n`;
    }
    if (tablet.lineheight.display?.display) {
      css += `  --line-display-display-tablet: ${tablet.lineheight.display.display}px;\n`;
    }
    
    // Heading tokens
    if (tablet.fontsize.heading) {
      Object.entries(tablet.fontsize.heading).forEach(([key, value]) => {
        css += `  --font-heading-${key.toLowerCase()}-tablet: ${value}px;\n`;
      });
    }
    if (tablet.lineheight.heading) {
      Object.entries(tablet.lineheight.heading).forEach(([key, value]) => {
        css += `  --line-heading-${key.toLowerCase()}-tablet: ${value}px;\n`;
      });
    }
  }
  
  // Desktop typography
  if (desktop.fontsize && desktop.lineheight) {
    css += '\n  /* Desktop Typography */\n';
    
    // Display tokens
    if (desktop.fontsize.display?.display) {
      css += `  --font-display-display-desktop: ${desktop.fontsize.display.display}px;\n`;
    }
    if (desktop.lineheight.display?.display) {
      css += `  --line-display-display-desktop: ${desktop.lineheight.display.display}px;\n`;
    }
    
    // Heading tokens
    if (desktop.fontsize.heading) {
      Object.entries(desktop.fontsize.heading).forEach(([key, value]) => {
        css += `  --font-heading-${key.toLowerCase()}-desktop: ${value}px;\n`;
      });
    }
    if (desktop.lineheight.heading) {
      Object.entries(desktop.lineheight.heading).forEach(([key, value]) => {
        css += `  --line-heading-${key.toLowerCase()}-desktop: ${value}px;\n`;
      });
    }
  }
...
function generateDarkModeCSS(mapped, primitivesDark) {
  let css = `/**
 * DARK MODE TOKENS
...
function getTotalTokenCount(mapped) {
  let count = 0;
