...
async function fetchFigmaNode(): Promise<FigmaResponse> {
...
async function main() {
  try {
    console.log('🔍 Fetching Figma component details...');
    ...
    const componentDetails = extractComponentDetails(node);
    ...
    const detailsPath = path.join(outputDir, `component-${NODE_ID}-details.json`);
    fs.writeFileSync(detailsPath, JSON.stringify(componentDetails, null, 2));
    console.log(`📄 Detailed component data saved to: ${detailsPath}`);

    // Save CSS mapping
    const cssPath = path.join(outputDir, `component-${NODE_ID}-css-mapping.json`);
    fs.writeFileSync(cssPath, JSON.stringify(cssMapping, null, 2));
    console.log(`🎨 CSS mapping saved to: ${cssPath}`);

    // Generate analysis report
    const reportPath = path.join(outputDir, `component-${NODE_ID}-analysis.md`);
    ...
    console.log('');
    console.log('📋 Summary:');
    console.log(`   Component Name: ${componentDetails.name}`);
    console.log(`   Component Type: ${componentDetails.type}`);
    console.log(`   Children Count: ${componentDetails.children?.length || 0}`);
    
    if (componentDetails.layout) {
      console.log(`   Layout Mode: ${componentDetails.layout.mode}`);
    }
...
function generateAnalysisReport(details: any, cssMapping: any): string {
  return `# Figma Component Analysis
